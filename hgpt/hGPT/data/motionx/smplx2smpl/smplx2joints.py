...
class SMPLX(object):
    def __init__(self):
        self.layer_arg = {'create_global_orient': False, 'create_body_pose': False, 'create_left_hand_pose': False, 'create_right_hand_pose': False,
                          'create_jaw_pose': False, 'create_leye_pose': False, 'create_reye_pose': False, 'create_betas': False, 'create_expression': False, 'create_transl': False}
...
def process_smplx_322_data(smplx_data, smplx_layer, smplx_model, device, face_carnical=False, pose=None, norm_global_orient=False, transform=False):
    ...
    if use_flame:
        ...
        body_parms = {
            ...
            'pose_body': pose[..., 3:3 + 63].to(device),
            # controls the finger articulation 90
            'pose_hand': pose[..., 66:66 + 90].to(device),
            # controls the yaw pose 3
            'pose_jaw': pose[..., 66 + 90:66 + 93].to(device),
            # controls the global body position 3
            'trans': pose[..., 159:159 + 3].to(device),
    ...
    if norm_global_orient:
        for i in range(num_frames):
            body_parms['root_orient'][:, i:i+1, :], body_parms['trans'][i:i+1] = compute_canonical_transform(
                body_parms['root_orient'][i:i+1], body_parms['trans'][i:i+1])
    ...
    if transform:
        ...
        body_parms['root_orient'] = compute_canonical_transform(
            body_parms['root_orient'])
...
if __name__ == '__main__':
    example_path = "datasets/Motion-X/motion_data/smplx_322/kungfu/subset_0/000006.npy"
    ...
    mesh = trimesh.Trimesh(vertices=vert.squeeze(
    ).detach().cpu().numpy()[50], faces=faces)
